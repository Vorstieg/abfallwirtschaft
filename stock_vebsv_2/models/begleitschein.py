...
class WasteBegleitschein(models.Model):
    ...
    company_partner_id = fields.Many2one('res.partner', string='Company', required=True, change_default=True, tracking=True)
    ...
    def start_begleitschein(self):
        ...
        company_gln = self._get_person_gln(self.company_partner_id,
    ...
    def pull_changes(self):
        ...
        edm_db_uuid = config_params.get_param('waste_management.edm_db_uuid')

        response = self._get_begleitschein_message_service().pull_news(edm_db_uuid)

        if response.db_uuid != edm_db_uuid:
            config_params.get_param('waste_management.edm_db_uuid', response.db_uuid)
