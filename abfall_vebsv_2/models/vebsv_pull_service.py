...
class VebsvPullService(models.TransientModel):
    ...
    def pull_changes_for_company(self, company):
        ...
        for line in respone["changes"]:
            ...
            elif line['state'] == 'BACKWARD_INFO':
                begleitscheine = self.env['waste.begleitschein'].search([
                    ("business_case_uuid", "=", line["begleitschein"]["business_case_uuid"])
                ])
                for bs in begleitscheine:
                    bs.message_post(body=line["message"], subtype_xmlid='mail.mt_note')
